...
| `LegacyPeerWire` | later | Caller: `scripts/verify-mesh-plane.ts` pairs two children with `initFleet` given **no** `nodeOrigin`, so invites advertise `httpOrigin`, pairing lands on the web server's legacy `/fleet/pair`, rows are written without `transport: "node"`, and the legacy wire carries the harness's push assertions. Phase 5 also keeps it deliberately. | `src/bun/fleet/wire.ts:43-145`, `:223-231`; harness `scripts/verify-mesh-plane.ts:98-109`, `:254-319`; `docs/federation.md:466-472` | R0 | — | |
